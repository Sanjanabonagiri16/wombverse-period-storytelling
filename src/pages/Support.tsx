  ...
  return (
    <Layout>
      <div className="container mx-auto px-4 py-12 max-w-3xl">
        <h1 className="text-3xl font-bold text-womb-indigo mb-4 font-playfair">Support Resources</h1>
        ...
        {isLoading && (
          <ul className="space-y-6">
            {[...Array(3)].map((_, i) => (
              <li key={i} className="bg-womb-darkgrey border border-womb-mediumgrey rounded-lg p-6">
        ...
        {resources && !isLoading && !isError && (
          <ul className="space-y-6">
            {resources.map((r) => (
              <li key={r.id} className="bg-womb-darkgrey border border-womb-mediumgrey rounded-lg p-6 transition-all duration-300 hover:-translate-y-1 hover:shadow-lg hover:border-womb-maroon/20">
                <h2 className="font-bold text-womb-white text-lg">{r.name}</h2>
                <p className="text-womb-lightgrey">{r.description}</p>
                <a className="mt-2 inline-block text-womb-indigo hover:underline" href={r.url} target="_blank" rel="noopener noreferrer">
                  Learn more
                </a>
              </li>
            ))}
